...
with open("1階ODE数値結果/NEWTON_EXACT(number=" + str(number) + ",h=" + str(
        h) + ",n=" + str(n) + ").csv", newline="") as f:
    reader = csv.reader(f)
    for i, row in enumerate(reader):
...
plt.xlabel("x")
plt.ylabel("y")
fig.savefig("1階ODE数値結果/plot(number=" + str(number) + ",reg_qubits=" + str(reg_nbit_list) + ",h=" + str(
...
plt.yscale("log")
fig.savefig(
    "1階ODE数値結果/error(number=" + str(number) + ",reg_qubits=" + str(reg_nbit_list) + ",h=" + str(
        h) + ",n=" + str(n) + ").png", bbox_inches="tight")
plt.show()
